...
import { getAppAdministratorById } from 'apiSdk/app-administrators';
import { AppAdministratorInterface } from 'interfaces/app-administrator';

function AppAdministratorViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AppAdministratorInterface>(
    () => (id ? `/app-administrators/${id}` : null),
    () =>
      getAppAdministratorById(id, {
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'App Administrator Details',
              isCurrent: true,
            },
          ]}
        />
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('app_administrator', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/app-administrators/edit/${id}`} passHref legacyBehavior>
                    <Button
