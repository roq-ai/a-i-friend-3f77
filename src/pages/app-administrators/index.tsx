...
import { getAppAdministrators, deleteAppAdministratorById } from 'apiSdk/app-administrators';
import { AppAdministratorInterface } from 'interfaces/app-administrator';

type ColumnType = ColumnDef<AppAdministratorInterface, unknown>;

interface AppAdministratorListPageProps {
...
export function AppAdministratorListPage(props: AppAdministratorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AppAdministratorInterface>>(
    () => `/app-administrators?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteAppAdministratorById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: AppAdministratorInterface) => {
    if (hasAccess('app_administrator', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/app-administrators/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/app-administrators/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('app_administrator', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/app-administrators/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('app_administrator', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="app_administrator" />
        </Flex>

        {hasAccess('app_administrator', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/app-administrators/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
