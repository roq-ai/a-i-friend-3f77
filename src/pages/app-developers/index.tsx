...
import { getAppDevelopers, deleteAppDeveloperById } from 'apiSdk/app-developers';
import { AppDeveloperInterface } from 'interfaces/app-developer';

type ColumnType = ColumnDef<AppDeveloperInterface, unknown>;

interface AppDeveloperListPageProps {
...
export function AppDeveloperListPage(props: AppDeveloperListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AppDeveloperInterface>>(
    () => `/app-developers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteAppDeveloperById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: AppDeveloperInterface) => {
    if (hasAccess('app_developer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/app-developers/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/app-developers/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('app_developer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/app-developers/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('app_developer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="app_developer" />
        </Flex>

        {hasAccess('app_developer', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/app-developers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
...
export default compose(
  ...
  withAuthorization({
    ...
    entity: 'app_developer',
    operation: AccessOperationEnum.READ,
  }),
  withAppLayout(),
)(AppDeveloperListPage);
