...
type ColumnType = ColumnDef<SupportStaffInterface, unknown>;

interface SupportStaffListPageProps {
...
export function SupportStaffListPage(props: SupportStaffListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SupportStaffInterface>>(
    () => `/support-staffs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SupportStaffInterface) => {
    if (hasAccess('support_staff', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/support-staffs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/support-staffs/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="support_staff" />
        </Flex>
