...
import { getAppAdministrators } from 'apiSdk/app-administrators';
import { AppAdministratorInterface } from 'interfaces/app-administrator';
import { getSupportStaffs } from 'apiSdk/support-staffs';
import { SupportStaffInterface } from 'interfaces/support-staff';
import { getAppDevelopers } from 'apiSdk/app-developers';
import { AppDeveloperInterface } from 'interfaces/app-developer';

import { AppAdministratorListPage as ShowCaseList } from 'pages/app-administrators';

function DashboardPage() {
  const { session } = useSession();

  const appAdministratorStat = useSWR<PaginatedInterface<AppAdministratorInterface>>(
    () => `/app-administrators`,
    () => getAppAdministrators({ limit: 1, offset: 0 }),
  );

  const supportStaffStat = useSWR<PaginatedInterface<SupportStaffInterface>>(
    () => `/support-staffs`,
    () => getSupportStaffs({ limit: 1, offset: 0 }),
  );

  const appDeveloperStat = useSWR<PaginatedInterface<AppDeveloperInterface>>(
    () => `/app-developers`,
    () => getAppDevelopers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'App Administrators',
      result: appAdministratorStat?.data?.totalCount,
      isLoading: appAdministratorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Support Staffs',
      result: supportStaffStat?.data?.totalCount,
      isLoading: supportStaffStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'App Developers',
      result: appDeveloperStat?.data?.totalCount,
      isLoading: appDeveloperStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest App Administrators
                </Text>
                <NextLink href={`app-administrators`} passHref legacyBehavior>
                  <Button
